...
source("/functions/asymptotic_variance.R")
...
Stations : 69028 (PILFs = 5), 71011 (PILFs = 2), 50014 (PILFs = 5), 10001 (PILFs = 2)
...
sd_mle <- sqrt(asymptotic_variance_mdpde(param_mle[1],param_mle[2],param_mle[3],alpha =0)[c(1,5,9)])/n^(1/2)
...
sd_mdpde01 <- sqrt(asymptotic_variance_mdpde(param_mdpde01[1],param_mdpde01[2],param_mdpde01[3],alpha =0.1)[c(1,5,9)])/n^(1/2)
...
sd_mdpde03 <- sqrt(asymptotic_variance_mdpde(param_mdpde03[1],param_mdpde03[2],param_mdpde03[3],alpha =0.3)[c(1,5,9)])/n^(1/2)
...
sd_mle_noPILFs <- sqrt(asymptotic_variance_mdpde(param_mle_noPILFs[1],param_mle_noPILFs[2],param_mle_noPILFs[3],alpha = 0)[c(1,5,9)])/length(data_noPILFs)^(1/2)
...
wasserstein_list <- list(
  MLE       = wasserstein_for_gev(params_list$MLE_noPILFs,params_list$MLE,1,0.01),
  MDPDE01   = wasserstein_for_gev(params_list$MLE_noPILFs,params_list$MDPDE01,1,0.01),
  MDPDE03   = wasserstein_for_gev(params_list$MLE_noPILFs,params_list$MDPDE03,1,0.01)
  )
