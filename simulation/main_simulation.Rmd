...
plt <- ggplot(df_plot, aes(x = x, y = y, color = group, shape = group)) +
  ...
  labs(x = TeX("$\\xi_1$"), y = TeX("$W_2$"), color = "Models")+
  scale_x_continuous(
    breaks = c(-2,-1.5,-1, -0.5, 0, 0.3, 1,1.5,2),
    labels = c("-2","-1.5","-1","-0.3","0","0.3","1","1.5","2")
  )+
  annotate("text", x = df_wd$xi0 + 0.1, y = 2*df_wd$wd_mq[15],
...
load("generated_datasets/df_wd_n100_sign_xi1_varying_sig_eps20.RData")
...
plt <- ggplot(df_plot, aes(x = x, y = y, color = group, shape = group)) +
  ...
  labs(x = TeX("$\\sigma_1$"), y = TeX("$W_2$"), color = "Models")+
...
load("generated_datasets/df_wd_n50_sign_xi0_varying_xi_eps10.RData")
...
plt <- ggplot(df_plot, aes(x = x, y = y, color = group, shape = group)) +
  ...
  annotate("text", x = df_wd$xi0 + 0.1, y = 1.5*df_wd$wd_mq[15],
           label = TeX("$\\xi_0$"), color = 'grey24',size=6)+
  theme_minimal() +
  scale_color_manual(
      values = c("MLE" = "#31123b",
